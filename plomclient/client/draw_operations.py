    ...
    def _get_arrow_flag(self, event):
        ...
        if (event.button() == Qt.MouseButton.RightButton) or (
            ...
            return 2 
             ...
             QGuiApplication.queryKeyboardModifiers() == Qt.KeyboardModifier.ControlModifier
        ):
            return 4 
        else:
            return 1 
...
class BoxToolDrawer(MultiStageDrawer):
    ...
    def __init__(self, scene, event):
        ...
        if self.box_flag == 1: # Rectangle
            self.temp_item = QGraphicsRectItem(QRectF(self.origin_pos, self.current_pos).normalized())
        elif self.box_flag == 2: # Ellipse
            self.temp_item = QGraphicsEllipseItem(QRectF(self.origin_pos.x(), self.origin_pos.y(), 0, 0))
    ...
    def mouse_move(self, event):
        ...
        if self.box_flag == 1: 
            ...
            rect = QRectF(self.origin_pos.x() - rx, self.origin_pos.y() - ry, 2*rx, 2*ry)
    ...
    def mouse_release(self, event):
        ...
        if self.temp_item:
            ...
            if final_rect.width() > self.minimum_side_length and final_rect.height() > self.minimum_side_length:
                if self.box_flag == 1:
...
class RubricToolDrawer(MultiStageDrawer):
    ...
    def _stamp_rubric(self):
        ...
        if not self.scene.isLegalRubric(self.scene.current_rubric) or self.scene.textUnderneathPoint(pt):
            return
    ...
    def mouse_press(self, event):
        if self.state == 0: 
            ...
            self.state = 1
            self.origin_pos = event.scenePos()
            self.temp_box_item = QGraphicsRectItem(QRectF(self.origin_pos, self.origin_pos))
    ...
    def mouse_move(self, event):
        ...
        if self.state == 1: 
            if self.temp_box_item:
                self.current_pos = event.scenePos()
                self.temp_box_item.setRect(QRectF(self.origin_pos, self.current_pos).normalized())
        elif self.state == 2: 
            if self.path_item and self.permanent_box_item:
                self.current_pos = event.scenePos()
                ghost_rect = self.scene.ghostItem.mapRectToScene(self.scene.ghostItem.boundingRect())
                box_rect = self.permanent_box_item.mapRectToScene(self.permanent_box_item.boundingRect())
    ...
    def mouse_release(self, event):
        if self.state == 1: 
            ...
            if final_rect.width() < self.minimum_side_length or final_rect.height() < self.minimum_side_length:
                self._stamp_rubric()
                self._finish()
            else:
...
class DeleteToolDrawer(MultiStageDrawer):
    ...
    def __init__(self, scene, event):
        ...
        self.temp_box_item = QGraphicsRectItem(QRectF(self.origin_pos, self.current_pos).normalized())
    ...
    def mouse_release(self, event):
        ...
        if not self.is_dragging: # Simple click
            nearby_rect = QRectF(self.origin_pos.x() - 5, self.origin_pos.y() - 5, 10, 10)
            items_to_check = self.scene.items(nearby_rect, mode=Qt.ItemSelectionMode.IntersectsItemShape)
            if items_to_check:
            ...
            items_to_check = self.scene.items(self.temp_box_item.rect(), mode=Qt.ItemSelectionMode.ContainsItemShape)
            for item in items_to_check:
...
class ZoomToolDrawer(MultiStageDrawer):
    ...
    def mouse_release(self, event):
        ...
        if not self.is_dragging or (self.temp_box_item.rect().height() < 8 and self.temp_box_item.rect().width() < 8):
            self.scene.views()[0].scale(1.25, 1.25)
            self.scene.views()[0].centerOn(event.scenePos())
        else:
            self.scene.views()[0].fitInView(self.temp_box_item, Qt.AspectRatioMode.KeepAspectRatio)
...
class CropToolDrawer(MultiStageDrawer):
    ...
    def mouse_release(self, event):
        ...
        if self.temp_box_item.rect().height() >= minbox and self.temp_box_item.rect().width() >= minbox:
